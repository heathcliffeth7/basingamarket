<script lang="ts">
  ...
  type Status = 'connecting' | 'live' | 'refetching' | 'offline' | 'mock';

  let { status = 'live', label }: { status?: Status; label?: string } = $props();
  ...
  const display = $derived(label ?? (status === 'mock' ? 'MOCK FALLBACK ACTIVE' : status));
</script>

<span class={cn('mono-label inline-flex items-center gap-2 rounded-full border px-2.5 py-1', classes[status])}>
  <span class={cn('h-1.5 w-1.5 rounded-full', dots[status])}></span>
  {display}
</span>
